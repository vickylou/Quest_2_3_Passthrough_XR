...
describe('computeBalances', () => {
  it('matches the hand calculation for the V0 baseline', () => {
    ...
    //                + 0.275 + 0.26 = 4.955 M.
    // (Agri land defaults to 6 500 m² × 40 €/m² = 260 000 €; per-sister
    //  spots all start at 0 so the agri land contributes to totalAssets
    //  but not to any single sister's balance until plots are assigned.
    //  Helmhaus renovation costs are additive on top of the headline
    //  value and split per the parent's allocation.)
    expect(b.totalAssets).toBeCloseTo(4_955_000, 0);
    expect(b.equalTarget).toBeCloseTo(1_238_750, 0);
